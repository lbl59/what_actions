...
# to modify ##########################################################
N_reals = 20  # number of realizations
...
def rof_evaluation(s_0, demand, inflows, evap, r):
    ...
    sp = inflows.shape[1] - demand.shape[1] - 42
    print("Realization = ", r)
    for w in range(n_weeks, n_sym_weeks):
        ...
        for n in range(n_sym_years):
            # evap_year and inflow_year should have length 52
            evap_year = evap[r, (n*n_weeks)+sp : ((n+1)*n_weeks)+sp]
            inflow_year = inflows[r, (n*n_weeks)+sp :  ((n+1)*n_weeks)+sp]
            s_t = s_0
...
ax.set_xlabel(r"Increasing reliability $\rightarrow$")
ax.set_ylabel(r"$\leftarrow$ Decreasing restriction frequency")
ax.set_title(r"Tradeoff between reliability and average restriction frequency \n 100 realizations")
